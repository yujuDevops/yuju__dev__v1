export const dataCardBici = [
  {
    numero: 1,
    tituloStrong: "Elegí el plan",
    tituloNormal: "que mas te guste",
    isStrongFirst: true,
    imagen: "https://res.cloudinary.com/dkk8nbi3b/image/upload/v1721740508/web_Icono-selecciona_azul_3_n8ekxs.svg",
  },

  {
    numero: 2,
    tituloStrong: "Ingresá tus datos personales",
    tituloNormal: "y método de pago.",
    isStrongFirst: true,
    imagen: "https://res.cloudinary.com/dkk8nbi3b/image/upload/v1721740603/web_Icono-pago_azul_2_pp8ygy.svg",
  },
  {
    numero: 3,
    tituloStrong: "Ingresá los datos y fotos",
    tituloNormal: "de tu bici.",
    isStrongFirst: true,
    imagen: "https://res.cloudinary.com/dkk8nbi3b/image/upload/v1721740383/web_Icono-completar_azul_3_k68cnn.svg",
  },
  {
    numero: 4,
    tituloStrong: "¡Listo!",
    tituloNormal: "Vas a recibir por email el certificado de cobertura.",
    isStrongFirst: true,
    imagen: "https://res.cloudinary.com/dkk8nbi3b/image/upload/v1721740627/web_Icono-asegurado_azul_3_ylxoce.svg",
  },
  ]
...
const PlanModal = ({ isOpen, onRequestClose, planDetails, handleClick, datos, handleSubmit, handleChange }) => {
  ...
  const handleContactClick = () => {
    ...
    handleClick(); // Llamar la función existente para manejar el click
  };

     // Ocultar el alert después de 3 segundos y reiniciar la página
     setTimeout(() => {
      setShowAlert(false);
  ...
  return (
    ...
    >
      ...
      {/* Mostrar el alert en la esquina inferior derecha */}
      {showAlert && (
  ...
  return (
    <div className="cardsHogar">
      <div className="cardHogar">
        ...
        <p className="pCardHogar__price">
          <strong>{planData[plan].pack300.price}</strong>/ mes
        </p>
        <button
          className="buttonCardHogar"
          onClick={() => openModal(plan300Details)}
        >
          Contratar
        </button>
      </div>
      <div className='separtor-hogar'></div>
      <div className="cardHogar">
        <h3 className="h3CardHogar">{planData[plan].pack500.title}</h3>
        <p className="pCardHogar">{planData[plan].pack500.description}</p>
        <p className="pCardHogar__price">
          <strong>{planData[plan].pack500.price}</strong>/ mes
        </p>
        <button
          className="buttonCardHogar"
          onClick={() => openModal(plan500Details)}
        >
          Contratar
        </button>
      </div>
      ...
      <div className="cardHogar">
        ...
        <p className="pCardHogar__price">
          <strong>{planData[plan].pack750.price}</strong>/ mes
        </p>
        <button
          className="buttonCardHogar"
          onClick={() => openModal(plan750Details)}
        >
          Contratar
        </button>
      </div>
